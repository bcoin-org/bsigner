...
class TrezorDeviceWrapper extends DeviceWrapper {
  ...
  async signTransaction(mtx, options) {
    ...
    for (let i = 0; i < optionPaths.length; i++) {
      ...
      if (Array.isArray(optionPaths[i])) {
        paths[i] = Path.fromList(optionPaths[i]);
        continue;
      }

      if (typeof optionPaths[i] === 'string') {
        paths[i] = Path.fromString(optionPaths[i]);
        continue;
      }
    }
...
function asserTrezorResponse(response) {
  if (response.success)
    return;
