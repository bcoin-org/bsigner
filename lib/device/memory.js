/*!
 * memory.js - Memory device, used for mock.
...
class MemoryDevice extends AbstractDevice {
  constructor(options) {
    super();
  ...
  fromOptions(options) {
    ...
    if (options.logger != null) {
      ...
      this.logger = options.logger.context('memory-device');
    }
    ...
    if (options.key != null) {
      // HDPrivateKey.isHDPrivateKey(options.key)
      assert(typeof options.key === 'object')
      key = options.key;
    }
