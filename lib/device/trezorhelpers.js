...
function sortKeysAndSignatures(multisig, network) {
  ...
  combined.sort((a, b) => {
    ...
    const pathA = common.parsePath(a[0].path).toString();
    const pathB = common.parsePath(b[0].path).toString();
...
function processMultisigInputData(inputData, network) {
  ...
  const [sortedPubkeys, sortedSignatures] = sortKeysAndSignatures(multisig, network);
  const signatures = [];
  ...
  for (const pubkey of sortedPubkeys) {
    ...
    const path = common.parsePath(pubkey.path);
