...
const AbstractDeviceWrapper = require('../device/abstract');
const LedgerDeviceManager = require('./ledger');
const TrezorDeviceManager = require('./trezor');
const MemoryDeviceManager = require('./memory');
...
class DeviceManager extends EventEmitter {
  ...
  async selectDevice(vendor, device) {
    if (!device && vendor instanceof AbstractDeviceWrapper) {
      device = vendor;
    ...
    if (this.selected && this.selected.vendor !== vendor)
      await this.vendorManagers.get(this.selected.vendor).deselectDevice();

    const selected = await this.vendorManagers.get(vendor).selectDevice(device);
    return selected;
  }
