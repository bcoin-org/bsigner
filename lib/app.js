 ...
 *
 * @param {bsigner#Manager} - hardware
 * @param {bsigner#Path} - path
 * @param {String} - enc
 * @returns {Buffer|String}
 */
async function generateToken(manager, path, enc) {
  if (!path)
 ...
 *
 * @param {bsigner#Manager}
 * @param {bclient#WalletClient#wallet}
 * @param {bcoin#Network|String}
 * @returns {bsigner#Path}
 */
async function guessPath(manager, wallet, network) {
  let target;

  assert(manager, 'must pass hardware');
  ...
  for (const key of keys.values()) {
    ...
    const xkey = await manager.getPublicKey(path);
 ...
 * @returns {Object}
 */
async function getKnownPaths(manager, wallet) {
  const out = {
    keys: [],
    paths: {}
  };
  ...
  for (const key of keys.values()) {
    ...
    for (const purpose of [44, 48]) {
      ...
      const xkey = await manager.getXPUB(path);
